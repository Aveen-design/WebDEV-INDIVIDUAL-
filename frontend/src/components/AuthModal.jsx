...
const AuthModal = ({ mode, onClose, onSwitch }) => {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    setError('');
    if (!formData.full_name || !formData.email || !formData.password) return setError('Please fill in all required fields');
            ...
            <>
              ...
              <form onSubmit={handleLogin} style={s.form}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <span style={s.forgotLink} onClick={() => { onClose(); navigate('/forgot-password'); }}>
                    Forgot password?
                  </span>
                </div>
...
const s = {
  ...
  visualOverlay: {
    position: 'absolute',
    inset: 0,
    background: 'linear-gradient(160deg, rgba(20,33,61,0.6) 0%, rgba(29,53,87,0.5) 100%)',
  },
  visualNav: {
    position: 'relative',
    zIndex: 1,
  ...
  subheading: { color: '#888', fontSize: '0.9rem', margin: '0 0 1.4rem' },
  errorBox: {
    background: '#fdecea',
  ...
  divider: { display: 'flex', alignItems: 'center', gap: '0.6rem', margin: '1rem 0' },
  ...
  footerText: { textAlign: 'center', color: '#888', fontSize: '0.875rem', marginTop: '1.1rem' },
  switchLink: { color: '#60A5FA', fontWeight: '600', cursor: 'pointer' },
};
